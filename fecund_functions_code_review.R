...
quant_gen_offspring_distribution <- function(N_f, eval_points, offspring_sd, seed_eval_points, dg){
  ...
  #possibly do away this this line and simply use a vector 
...
#potentially faster way to use dnorm in line 36
plot(matrix(dnorm(rep(seq(-10, 20, 0.5), each = 2), c(1, 10), 0.5), nrow = 2, byrow = F)[2,])
...
speed_test = microbenchmark(fecundity(N_m = dist_parents_low, eval_points = eval_points_low_res$above_ground, fec_max = 100, fec0 = 0, fec_cost = 1, N_f = dist_parents_low, 
  ...
  times = 100)
speed_test #Note how badly this function scales as the resolution increases, res of 0.5 is about as course as I want to go. Above this the integrations using dg (e.g. 64) starts to become innaccurate   

#faster dnorm function
require(Rcpp)
setwd('/home/shauncoutts/Dropbox/Rfiles')
sourceCpp("my_dnorm.cpp")
my_dnorm(x = seq(-10, 10, 0.5), means = 1, sd = 0.7)
plot(my_dnorm(x = seq(-10, 10, 0.5), means = 1, sd = 0.7), dnorm(x = seq(-10, 10, 0.5), mean = 1, sd = 0.7))