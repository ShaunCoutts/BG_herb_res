...
function exper_iter(int_rr_1::Array{Float64, 1}; int_rr_2::Array{Float64, 1}, 
...
function landscape_setup(g_vals::Array{Float64, 1}, x_dim::Int64, burnin::Int64)
  
  # set up the three sink landscapes, empty, naive and exposed    
  RR_int_ls_empty = zeros(length(g_vals), x_dim)
  Rr_int_ls_empty = zeros(length(g_vals), x_dim)
  rr_int_ls_empty = zeros(length(g_vals), x_dim)
  
  RR_int_ls_naive = zeros(length(g_vals), x_dim)
  Rr_int_ls_naive = zeros(length(g_vals), x_dim)
  rr_int_ls_naive = zeros(length(g_vals), x_dim)

  RR_int_ls_expos = zeros(length(g_vals), x_dim)
  Rr_int_ls_expos = zeros(length(g_vals), x_dim)
  rr_int_ls_expos = zeros(length(g_vals), x_dim)

  # fill the naive and exposed landscapes
  for x in 1:x_dim
    RR_int_ls_naive[:, x] = pdf(Normal(int_g, int_sd), g_vals) * int_num_RR
    Rr_int_ls_naive[:, x] = pdf(Normal(int_g, int_sd), g_vals) * int_num_Rr
    rr_int_ls_naive[:, x] = pdf(Normal(int_g, int_sd), g_vals) * int_num_rr
    
    RR_int_ls_expos[:, x] = pdf(Normal(int_g, int_sd), g_vals) * int_num_RR
    Rr_int_ls_expos[:, x] = pdf(Normal(int_g, int_sd), g_vals) * int_num_Rr
    rr_int_ls_expos[:, x] = pdf(Normal(int_g, int_sd), g_vals) * int_num_rr
  end

  # let the populations evolve for burnin 
end
