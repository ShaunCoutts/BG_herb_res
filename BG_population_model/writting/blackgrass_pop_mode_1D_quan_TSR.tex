...
Since the early 20th century evolutionary biologists have recognized a continuum in the genetic architecture of traits, from traits controlled by a single loci at one extreme, to polygenic, quantitative traits at the other extreme, which are controlled by the additive affect of many genes \citep{Land1989, Mack2009, Rajo2013}. There is growing evidence that within the same population a single complex trait can be conferred by both genetic architectures \citep{Donn2009, Bing2011, Hend2013, Oake2013, Bauc2016}. The implications for the evolution of these multi-architecture traits remains relatively unexplored (see \citealt{Deba2015}, \citealt{Yeam2015} for exceptions). In addition to theoretical interest these dual architecture traits also have important practical implications. On of the best examples of a dual architecture trait is herbicide and pesticide resistance. Together, these chemical tools are a crucial part of the worlds food productions system (ref), and are important for controlling insect spread diseases \citep{Nkya2013}. However, resistance to key chemical control methods threatens the continued use of these important tools \citep{Barb2011, Nkya2013}. Compounding this problem for weed species, economic and regulatory conditions have meant that no new classes of herbicide have been brought to market over the last two decade \citep{Duke2012}. This makes using the chemical tools we do have in a way that extends their useful life more important than ever. In order to do this we first need to understand how target site and quantitative resistance interact to drive the evolution and spread of resistant genotypes through populations. 

In the context of herbicide and pesticide resistance, resistance conferred by mutation in a single loci is known as target site resistance. Quantitative resistance (controlled by many genes) is usually  achieved by metabolizing the poison, or transporting the poison away from its binding site \citep{Bauc2016}. Although it is currently unknown how these two genetic architectures behind resistance interact, there is some evidence from misquote control programs that low levels of resistance conferred via an inefficient pathway can increase the rate of evolution for a much less costly target site mutation that confers high levels of resistance \citep{Vera2015}. 


In order for resistance (or any trait) to evolve in a population there first must be standing variation in that trait for natural selection to act on.      

competition and dispersal can interact to speed up fixation of resistance in bacteria \citep{Herm2012}. 

(\textit{sensu} \citealt{Deba2015} the number and effect size of loci contributing to divergence) 
