...
Since the 1970's evolutionary biologists have recognized a continuum in the way that traits are inherited, at one extreme is Mendelian inheritance where discreet traits are controlled by a single loci (e.g. wrinkly vs smooth skinned peas), at the other extreme are quantitative traits, which are controlled by the additive affect of many genes, resulting in continuous, normally distributed traits (e.g. height). However it is becoming increasingly apparent that a single complex trait can be influenced by both genetic architectures. The implications for the evolutions of these multi-architecture traits remains relatively unexplored. In addition to theoretical interest these dual architecture traits also have important practical implications.     
