...
seedbank_plowing <- function(seedbank0, seed_survival, germination, seed_movement, deep_germ_reduce){
...
emergence <- function(seedbank_current, germination, deep_germ_reduce){
  seedbank_current[1, ] * germination + seedbank_current[2, ] * germination * deep_germ_reduce
}
...
single_iteration <- function(seedbank_current, germination, eval_object, herb_rate, sur0, sur_cost_resist, herb_effect, survive_resist, max_sur, ceiling_pop, seed_survival, fec_max, fec0, fec_cost, additive_variance, density_cutoff){
  #actions effects on parameters
  germ_rate = sub_action_germ[action[1]]
  
  
  #processes
  new_seedbank = seedbank_plowing(seedbank_current, seed_survival, germination, seed_movement, deep_germ_reduce) 
  new_plants = emergence(seedbank_current = new_seedbank, germination = germination, deep_germ_reduce = deep_germ_reduce)
  ...
  new_seeds = seedbank(seedbank0 = seedbank_current, seed_survival = seed_survival, germination = germination, eval_object = eval_object, 
...
#visulisation that shows seedbank and above ground population
population_vis <- function(results_list, eval_points, output_loc){
  num_iterations = sqrt(length(results_list))
  plot_matrix_dims = c(ceiling(num_iterations), floor(num_iterations)) 
  setwd(output_loc)
  pdf(file = 'IMW_population_vis.pdf', height = 20, width = 15)
    par(mfrow = c(plot_matrix_dims))
    for(i in num_iterations){
      plot(eval_points, results_list[[i]][1, ], col = 'blue', type = 'l')
      lines(eval_points, results_list[[i]][2, ], col = 'red')
    }
  dev.off()
}
...
test_functions_broken <- function(file_loc){
  ...
  test_sb = seedbank_plowing(seedbank0 = test_sb0, seed_survival = 1, germination = 0, seed_movement = 0.2, deep_germ_reduce = 0)
  start_pop_top = sum(test_sb0_top * 0.1)
  start_pop_bottom = sum(test_sb0_bottom * 0.1)
  ...
  test_rsults[10] <- ifelse(test_seedbank_num_constant & test_seedbank_levels, 'Two layer SEEDBANK() function numbers add up', 'Something broke the two layer SEEDBANK() and numbers no longer correct')
  print(test_results) 
}
